...
<?php 
	if(isset($_POST['add'])) {
		...
		if(! $conn ) { die('Could not connect: ' . mysql_error()); 
			...
			if(!$blood_group ){
				echo "<div class='alert alert-info' style='margin-top:2px; margin-bottom:2px;'><h3> Please Entered Blood group </h3></div>";
				
			}if(!$status ){
				echo "<div class='alert alert-info' style='margin-top:2px; margin-bottom:2px;'><h3> Please Entered status </h3></div>";
				
			}if(!$blood_donor_id ){
				echo "<div class='alert alert-info' style='margin-top:2px; margin-bottom:2px;'><h3> Please Entered blood_donor </h3></div>";
				exit();
			}
		...
		VALUES (NULL, '$blood_group', '$status', '$blood_donor_id');";
...
?> 
	<form method="post" class="form-horizontal" action="<?php $_PHP_SELF ?>"> 
		...
		<div class="form-group"> 
			...
			<div class="col-sm-6"> 
				<select class="form-control"  name="blood_group" type="text" >
				<option></option>
				<option>A+</option>
				<option>A-</option>
				<option>B+</option>
		...
		<div class="form-group"> 
			<label for="firstname" class="col-sm-3 control-label">Donor's First Name</label> 
			<div class="col-sm-6"> 
				<select class="form-control" name="blood_donor_id" type="text" >
					<option>Select Donor's First Name</option>
					<?php
						$donor='SELECT * FROM blood_donor';
						$donorname=mysql_query($donor);
						while($name=mysql_fetch_array($donorname)){
		...
		<div class="form-group"> 
			<div class="col-sm-offset-2 col-sm-10"> 
				<button name="add" class="btn btn-primary" type="submit" id="add" value="">Register Blood on stock</button>
			</div> 
		</div> 
	</form> 
	<?php } ?> </div></div>
